...
public class customerPanel extends JPanel implements TableModelListener{
	JTable table,comTable;
	comModel comModel;
	customerModel tableModel;
	...
	public customerPanel(UserVO user){
		list=cbs.showCommodity();
		salelist = sbs.showSales();
		customerList = new ArrayList<String>();
		for(SalesVO sv:salelist){
			if(!customerList.contains(sv.getCustomer()))
				customerList.add(sv.getCustomer());
		}
		...
		scrollPane.setBounds(10, 21, 106, 190);
		add(scrollPane);
		
		JScrollPane scrollPane2 = new JScrollPane(comTable);
		scrollPane2.setBounds(126, 21, 106, 190);
		add(scrollPane2);
		
		delButton.addActionListener(new RatioListener());
		
		if(list!=null){
			for(String vo:customerList){
				Vector v=new Vector();
				v.add(vo);
				tableModel.addRow(v);
		}}
		...
		add(label_3);
				...
				imagePanel.setBounds(290, 21, 535, 546);
				add(imagePanel);
...
//				dataset.addValue(num, name, time);
			...
			else if(row2==-1||table.getRowCount()==0){
				comList= new ArrayList<com>();
				...
				for(SalesVO sv:salelist){
					...
					String time = sv.getID();
					for(CommodityPO cp:tempList)
						if(sv.getCustomer().equals(customerName)){
						com c = new com(cp.getType(),cp.getAmount(),time);
						boolean has =false;
						int index=0;
						for(com co:comList){
...
//					dataset.addValue(num, name, time);
				...
				comList= new ArrayList<com>();
				String customerName = (String)table.getValueAt(row, 0);
				String commodityName = (String)comTable.getValueAt(row2, 0);
				for(SalesVO sv:salelist){
					tempList=sv.getCommodityList();
					String time = sv.getID();
					for(CommodityPO cp:tempList)
						if(sv.getCustomer().equals(customerName)&&cp.getName().equals(commodityName)){
						com c = new com(cp.getType(),cp.getAmount(),time);
						boolean has =false;
						int index=0;
						for(com co:comList){
...
//			updateTableTwo();
	...
	public class SelectionListener implements ListSelectionListener {
        ...
        public void valueChanged(ListSelectionEvent e) {
        	...
        	if(row==-1||table.getRowCount()==0)
        		return;
            cuName = (String)table.getValueAt(table.getSelectedRow(),0);
            updateTable2();
        }
    }
	...
	public void updateTable2() {
		...
		ArrayList<String> comName = new ArrayList<String>(); 
		for(SalesVO sv:salelist){
			if(sv.getCustomer().equals(cuName)){
				tempList = sv.getCommodityList();
				for(CommodityPO c:tempList){
					if((!comName.contains(c.getID()))&&(!c.getName().equals("赠品"))){
						comName.add(c.getID());
						list.add(c);
					}
				}
