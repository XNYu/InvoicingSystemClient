...
import presentation.tableGUI.CostBillPanel.redFlushListener;
...
public class ReceivingBillPanel extends JPanel implements TableModelListener,ListSelectionListener{
	JTable table,table2;
	BillModel tablemodel1;
	TransferModel tablemodel2;
	JComboBox<String> customerBox;
	JComboBox<String> userBox;
	ReceivingBillPanel panel=this;
	...
	ArrayList<ReceivingBillVO> blist=new ArrayList<ReceivingBillVO>();
	ArrayList<ReceivingBillVO> templist=new ArrayList<ReceivingBillVO>();
	
	
	public ReceivingBillPanel(UserVO u){
		us=u;
		bill=new Bill(us);
		setSize(850, 460);
		setLayout(null);
		...
		tablemodel2=new TransferModel();
	 	...
	 	customer=bill.getCustomerlistOfSKD();
		...
		scrollpane2.setBounds(430, 38, 400, 402);
		add(scrollpane2);
	...
	class selectListener implements ListSelectionListener {
        ...
        public void valueChanged(ListSelectionEvent e) {
            ...
            ReceivingBillVO vo=blist.get(row);
            transferlist=vo.getTransferList();
	...
	class searchListener implements ActionListener{
		...
		public void actionPerformed(ActionEvent e) {
			...
			blist=bill.searchSKD(d1, d2, user, customer);
			updateTable();
		}
	}
