...
public class BillTable extends JPanel implements TableModelListener, ListSelectionListener{
	/**
	 * 
	 */
	...
	public BillTable(JLabel sumtext, final TransferList transferlist, BillBLService bill, boolean isModified){
		this.isModified = isModified;
		b=new BillModel(sumtext, transferlist, bill,isModified);
		if(isModified)
			t = new JTable(b);
		else{
			t = new JTable(b){ 
				...
				public void actionPerformed(ActionEvent e) {
					...
					if(selectedRows == null)
					    ...
					    for(int i = 0 ; i < selectedRows.length ;i++){
					    	//sumlabel.setText(Double.toString(transferlist.total()));
	...
	public void tableChanged(TableModelEvent e) {
        ...
        if(!model.getValueAt(model.getRowCount()-1, 0).equals("")&&!model.getValueAt(model.getRowCount()-1, 1).equals("")&&!model.getValueAt(model.getRowCount()-1, 2).equals("")){
        	transferlist.addTransfer(new TransferVO(bill.getAccount((String)model.getValueAt(model.getRowCount()-1, 0)),
