...
public class OverflowPanel extends JPanel implements ListSelectionListener{
	CommodityBLService cbs = new CommodityController();
	DocumentBLService dbs = (DocumentBLService) new DocumentController();
	JScrollPane overflowPane,comPane;
	DocOverTableModel overflowTableModel = new DocOverTableModel();
	DocComTableModel comTableModel = new DocComTableModel();
	int rowNum = 0;
	JTable overflowTable,comTable;
	JButton addOverflowButton = new JButton("ADD"),
			searchButton;
	...
	UserVO uvo;
	public OverflowPanel(UserVO uvo){
		this.uvo=uvo;
		addOverflowButton.setSize(150,20);
		addOverflowButton.setLocation(150,230);
		addOverflowButton.addActionListener(new addOverflowListener());
		this.add(addOverflowButton);
...
//		comTable.getColumnModel().getColumn(1).setPreferredWidth(10);
		...
		//====================================================================
		overflowTable = new JTable(overflowTableModel);
		overflowTable.getColumnModel().getColumn(0).setPreferredWidth(30);
...
//		stockTable.getModel().addTableModelListener(this);
		overflowPane = new JScrollPane(overflowTable);
	...
	public class addOverflowListener implements ActionListener{
		public void actionPerformed(ActionEvent arg0) {
			...
			if(isNumber(amountStr)){
				...
				ResultMessage rm = dbs.createOverflow((String)comTableModel.getValueAt(selectedRows[0], 1), 
				...
				if(rm==ResultMessage.Failure)
					return;
				cbs.changeAmount((String) comTableModel.getValueAt(selectedRows[0], 0), "DOCUMENT",amount);
				comTableModel.setValueAt(amount, selectedRows[0], 3);
				for(int i =overflowTable.getRowCount();i>0;i-- ){
					comTableModel.removeRow(0);
				}
				ArrayList<DocumentPO> list2 = dbs.showDocument("OVERFLOW");
				...
				loginterface.buildlog(uvo.getName()	, "新建报溢单："+"id"+(String)comTableModel.getValueAt(selectedRows[0], 0)
						+"系统数量:"+String.valueOf(comTableModel.getValueAt(selectedRows[0], 3))+
						"真实数量:"+amount);
				overflowTable.revalidate();
				overflowTable.repaint();
			}else{
